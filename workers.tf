...
resource "vsphere_virtual_machine" "worker" {
  count = length(var.vmw.kubernetes.clusters) * var.vmw.kubernetes.workers.count
  name             = "${var.vmw.kubernetes.clusters[floor(count.index / var.vmw.kubernetes.workers.count)].name}-worker-${floor(count.index / var.vmw.kubernetes.workers.count)}"
  ...
  vapp {
    properties = {
      hostname    = "${var.vmw.kubernetes.clusters[floor(count.index / var.vmw.kubernetes.workers.count)].name}-worker-${floor(count.index / var.vmw.kubernetes.workers.count)}"
