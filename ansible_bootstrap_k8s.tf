resource "null_resource" "ansible_hosts_cluster_master" {
  ...
  provisioner "local-exec" {
    command = "echo '---' | tee hosts_cluster_${count.index} ; echo 'all:' | tee -a hosts_cluster_${count.index} ; echo '  children:' | -a tee hosts_cluster_${count.index}; echo '    master:' | -a tee hosts_cluster_${count.index}; echo '      hosts:' | -a tee hosts_cluster_${count.index} ; echo '        ${vsphere_virtual_machine.master[count.index].default_ip_address}:' | tee -a hosts_cluster_${count.index}"
  }
}
...
resource "null_resource" "ansible_hosts_cluster_static1" {
  ...
  count            = length(var.vmw.kubernetes.clusters)
  provisioner "local-exec" {
    command = "echo '    workers:' | -a tee hosts_cluster_${count.index} ; echo '      hosts:' | -a tee hosts_cluster_${count.index}"
  }
}
...
resource "null_resource" "ansible_hosts_cluster_static2" {
  ...
  count            = length(var.vmw.kubernetes.clusters)
  provisioner "local-exec" {
    command = "echo '  vars:' | -a tee hosts_cluster_${count.index} ; echo '    ansible_user: ubuntu' | -a tee hosts_cluster_${count.index}; echo '    ansible_ssh_common_args: ${var.jump.private_key_path}' | tee -a hosts_cluster_${count.index}"
  }
}